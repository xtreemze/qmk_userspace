...
static bool settings_store_valid(const xtreemze_halcyon_settings_store_t *store) {
    return store->magic == XTREEMZE_HALCYON_SETTINGS_MAGIC &&
           store->version == XTREEMZE_HALCYON_SETTINGS_STORE_VERSION &&
           timings_valid(store->pattern_frame_ms, store->mod_recent_ms);
}
...
static void settings_sync_slave_handler(uint8_t in_buflen, const void *in_data, uint8_t out_buflen, void *out_data) {
    ...
    if (packet->protocol_version != XTREEMZE_HALCYON_SETTINGS_PROTOCOL_VERSION ||
        packet->size > XTREEMZE_HALCYON_SYNC_CHUNK_DATA ||
        (uint16_t)packet->offset + packet->size > sizeof(incoming_store)) {
        return;
    }
