...
grep -Eq '^#define[[:space:]]+XTREEMZE_DEFAULTS_EE_MARKER[[:space:]]+0xB0([[:space:]]|$)' "$keymap_file" ||
    fail "Expected the Vial defaults marker to match the deterministic direction profile (0xB0)."
