...
#define XTREEMZE_HALCYON_DISPLAY_CAP_FLAGS                                                                                      \
    (XTREEMZE_HALCYON_DISPLAY_CAP_LAYER_LABELS | XTREEMZE_HALCYON_DISPLAY_CAP_MODIFIER_LABELS |                              \
     XTREEMZE_HALCYON_DISPLAY_CAP_PATTERNS | XTREEMZE_HALCYON_DISPLAY_CAP_SPLIT_REPLICATION)

#define XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_DATA 27
#define XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_COUNT                                                                              \
    ((XTREEMZE_HALCYON_DISPLAY_EEPROM_SIZE + XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_DATA - 1) /                                  \
     XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_DATA)
...
_Static_assert(sizeof(xtreemze_halcyon_display_store_t) == XTREEMZE_HALCYON_DISPLAY_EEPROM_SIZE,
               "Halcyon display EEPROM reservation does not match store size");
...
static const char *const default_layer_labels[XTREEMZE_HALCYON_DISPLAY_LAYER_COUNT] = {
    "MOUSE", "QWERTY", "COLEMAK", "NUMSYMS", "NUMFLIP", "ONESHOT", "EDITING",
    "FNSYMS", "FNFLIP", "SYMBOLS", "RGBHUE", "RGBVAL", "BKLIGHT",
};
...
static inline void *display_store_eeprom_address(void) {
    return (void *)(uintptr_t)(TOTAL_EEPROM_BYTE_COUNT - XTREEMZE_RGB_PROFILE_EEPROM_SIZE - XTREEMZE_HALCYON_SETTINGS_EEPROM_SIZE -
                               XTREEMZE_HALCYON_DISPLAY_EEPROM_SIZE);
}
...
static bool display_pattern_valid(const xtreemze_halcyon_pattern_t *pattern) {
    return pattern->motif < XTREEMZE_HALCYON_DISPLAY_MOTIF_COUNT && pattern->tile_width >= XTREEMZE_HALCYON_TILE_MIN &&
           pattern->tile_width <= XTREEMZE_HALCYON_TILE_MAX && pattern->tile_height >= XTREEMZE_HALCYON_TILE_MIN &&
           pattern->tile_height <= XTREEMZE_HALCYON_TILE_MAX && pattern->motion_amplitude <= XTREEMZE_HALCYON_MOTION_MAX &&
           pattern->pulse_amplitude <= XTREEMZE_HALCYON_PULSE_MAX;
}
...
static bool display_store_valid(const xtreemze_halcyon_display_store_t *store) {
    ...
    for (uint8_t index = 0; index < XTREEMZE_HALCYON_DISPLAY_LAYER_COUNT; ++index) {
        if (!display_label_valid(store->layer_labels[index], XTREEMZE_HALCYON_LAYER_LABEL_SIZE) ||
            !display_pattern_valid(&store->patterns[index])) {
            return false;
        }
    }
...
static void display_sync_slave_handler(uint8_t in_buflen, const void *in_data, uint8_t out_buflen, void *out_data) {
    ...
    if (packet->protocol_version != XTREEMZE_HALCYON_DISPLAY_PROTOCOL_VERSION ||
        packet->size > XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_DATA ||
        (uint16_t)packet->offset + packet->size > sizeof(display_incoming_store)) {
        return;
    }
    ...
    if (chunk >= XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_COUNT ||
        packet->offset != chunk * XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_DATA) {
        return;
    }
...
static void display_sync_housekeeping(void) {
    ...
    if (!master || !connected || !display_sync_pending ||
        timer_elapsed32(display_last_sync_attempt) < XTREEMZE_HALCYON_DISPLAY_SYNC_INTERVAL_MS) {
        return;
    }
    ...
    const uint8_t size = remaining > XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_DATA ? XTREEMZE_HALCYON_DISPLAY_SYNC_CHUNK_DATA
                                                                                : (uint8_t)remaining;
